...
import { ControlsContainer, FullScreenControl, SigmaContainer, useLoadGraph, useRegisterEvents, useSigma } from "@react-sigma/core";
...
const SearchGraphSubplot: FC<SearchGraphSubplotProps> = ({ type, identity, name, handleNodeClicked, showMaterialInfo }) => {
    ...
    useEffect(() => {
      registerEvents({
        clickNode: (event) => {
          ...
          if (ENTITY_TYPES[type as keyof typeof ENTITY_TYPES] === ENTITY_TYPES.Material && data && name === data.name) {
  ...
  return (
    <SigmaContainer style={{ height: "400px", backgroundColor: "#F2E3D5" }} settings={{ renderEdgeLabels: true }}>
